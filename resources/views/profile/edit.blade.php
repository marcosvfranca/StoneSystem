@extends('layouts.app', ['activePage' => 'profile', 'titlePage' => __('Meu perfil')])
          ...
          <form method="post" action="{{ route('profile.update') }}" autocomplete="off" class="form-horizontal">
            ...
            <div class="card ">
              <div class="card-header card-header-warning">
                <h4 class="card-title">{{ __('Meu perfil') }}</h4>
              </div>
              <div class="card-body ">
                @if (session('status'))
                  <div class="row">
                ...
                @endif
                <div class="row">
                  <label class="col-sm-2 col-form-label">{{ __('Meu Nome') }}</label>
                  <div class="col-sm-7">
                    <div class="form-group{{ $errors->has('name') ? ' has-danger' : '' }}">
                      <input class="form-control{{ $errors->has('name') ? ' is-invalid' : '' }}" name="name" id="input-name" type="text" placeholder="{{ __('Meu Nome') }}" value="{{ old('name', auth()->user()->name) }}" required="true" aria-required="true" autofocus/>
                      @if ($errors->has('name'))
                ...
                <div class="row">
                  <label class="col-sm-2 col-form-label">{{ __('Meu Email') }}</label>
                  <div class="col-sm-7">
                    <div class="form-group{{ $errors->has('email') ? ' has-danger' : '' }}">
                      <input class="form-control{{ $errors->has('email') ? ' is-invalid' : '' }}" name="email" id="input-email" type="email" placeholder="{{ __('Meu Email') }}" value="{{ old('email', auth()->user()->email) }}" required />
                      @if ($errors->has('email'))
              ...
              <div class="card-footer ml-auto mr-auto">
                <button type="submit" class="btn btn-warning">{{ __('Alterar dados') }}</button>
              </div>
            </div>
          </form>
        </div>
      ...
      <div class="row">
        <div class="col-md-12">
          <form method="post" action="{{ route('profile.password') }}" class="form-horizontal">
            ...
            <div class="card ">
              <div class="card-header card-header-warning">
                <h4 class="card-title">{{ __('Alterar minha senha') }}</h4>
{{--                <p class="card-category">{{ __('Senha') }}</p>--}}
              ...
              <div class="card-footer ml-auto mr-auto">
                <button type="submit" class="btn btn-warning">{{ __('Alterar senha') }}</button>
              </div>
            </div>
          </form>
        </div>
